...
# from model_original import VisionTransformer
...
def valid_epoch(epoch, model, data_loader, criterion, metrics, device=torch.device('cpu')):
    ...
    acc5s = []
    # validation loop
    new_label = np.load("/storageStudents/ncsmmlab/tungufm/UIT-ADrone_Rain/data/rain_4/test/test_frame_mask/DJI_0065.npy")
...
save_path = '/storageStudents/ncsmmlab/tungufm/UIT-ADrone_Rain/ANDT/ASTT/experiments_andt_adrone_snow/'
...
def test_all_scenes(model, test_path, config, device=None):
    ...
    path_labels = os.path.join(test_path, 'test_frame_masks/')
    list_np_labels = []
    path_scenes.sort()
    losses = []
    ...
    print("Evaluation results:, AUC@1: {:.4f} - Mean loss: {:.2f}".format(frame_auc, loss_all))
    return frame_auc
...
def main():
    ...
    if bool(config.train):
        # Loading dataset
        train_folder = "/storageStudents/ncsmmlab/tungufm/UIT-ADrone/train/frames/"
        ...
        #--val test_folder = "/storageStudents/ncsmmlab/tungufm/UIT-ADrone-Snow/snow-data/test/frames/DJI_0065"

        #--test--#  
        test_folder = "/storageStudents/ncsmmlab/tungufm/UIT-ADrone_Rain/data/rain_4/test/frames/DJI_0073/"
        ...
        print('Testing ...')
        test_folder = "/storageStudents/ncsmmlab/tungufm/UIT-ADrone-Snow/snow-data/test"
