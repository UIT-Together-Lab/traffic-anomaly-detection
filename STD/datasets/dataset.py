...
def video_path_list(dataset_path):
    ...
    for video_path_iter in video_list:
        ...
        idx = idx+1
        # img_list.sort(key = lambda x: int(os.path.basename(x).split('.')[0][-6:]))
...
class ImageFolder(Dataset):
    def __init__(self, dataset_root_path, folder_path, video_num = 5, frame_interval = 1, img_size=256, rgb_rags = False):
        self.files = generate_trainfile(dataset_root_path, video_path=folder_path, video_num=video_num, frame_interval=frame_interval)
...
if __name__ == "__main__":
    batch_path_list = generate_trainfile( './', 'andt/UIT_ADrone_Foggy/train/frames')
