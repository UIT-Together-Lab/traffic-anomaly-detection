...
#     dataset_path = os.path.join(dataset_path,'test/frames')
...
#         idx = idx+1
#         img_list.sort(key = lambda x: int(os.path.basename(x).split('.')[0].split('_')[-1]))
...
def video_path_list(dataset_path):
    # dataset_path = os.path.join(dataset_path, 'test', 'frames')
    dataset_path = os.path.join(dataset_path, 'test', 'frames_foggy')
    print(f"Checking directory: {dataset_path}")  # Debug print
    
    if not os.path.isdir(dataset_path):
        print(f"Error: {dataset_path} is not a directory or does not exist.")
        return []
    ...
    for video_folder in video_list:
        ...
        img_list = os.listdir(video_folder_path)
        img_list = [os.path.join(video_folder_path, img) for img in img_list if img.lower().endswith(('.png', '.jpg', '.jpeg'))]
    ...
    print(f"Found {len(video_path_list)} video folders with images.")
    return video_path_list
...
class sliding_datasets(object):
    ...
    def clip(self, frame_idx):
        ...
        for idx in range(len(img_path)):
            ...
            # Track image name
            list_image_names.append(img_path[idx])
        batch = torch.cat(batch, 0)
        return batch, list_image_names

    def fetch(self):
        batches = []
        list_image_names = []
        for idx in range(self.batchsize):
            batch, list_image_name = self.clip(self.moving_idx)
            batches.append( torch.unsqueeze(batch, 0) )
            list_image_names.append(list_image_name)
            self.moving_idx += 1
        ...
        return batches, list_image_names
