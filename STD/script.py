...
def train(args):
    ...
    train_set = ImageFolder(config.dataset_path, 'train/', config.clips_length , config.frame_interval , config.img_size, config.rgb_tags)
    ...
    if args.pretrain_tag:
        for iter_idx in range(config.pretrain_batches):
    ...
    if args.finetune_tag:
        for iter_idx in range(config.total_batches):
            if (ts_idx + 2) >= training_nums:
                training_iter = iter(training_loader)
                ts_idx = 0
...
def eval_model(dataset_path, dataset_name, log_dir, model_type):
    data_name = 'adrone_foggy2'
    config.log_path = os.path.join(log_dir, dataset_name)
    config.dataset_path = dataset_path
    config.dataset_name = dataset_name
    
    checkpoint_paths = glob.glob(log_dir + '/**/*.pth')
    checkpoint_paths = sorted(checkpoint_paths)
    checkpoint_path = checkpoint_paths[-1]
    
    solver = Solver(config, cluster_model=PredRes_AE_Cluster_Model, model_type=model_type, checkpoint_path=checkpoint_path)

    solver.load_model()
    eval_loader, eval_labels = sliding_whole_dataset(config).generate_video_sequence()
    solver.para_tag = True
    solver.eval_datasets(eval_loader, eval_labels, 111, None, data_name)
    return

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Process some integers.')
    parser.add_argument('--gpu', type=int, default=1, help="selected gpu idx")
    parser.add_argument('--dataset_name', type=str, default='UIT_ADrone_Foggy', choices=['ped2', 'avenue', 'shanghai_tech'], help="selected datasets")
    parser.add_argument('--dataset_path', type=str, default='../UIT_ADrone_Foggy/', help="datasets root path ")
    parser.add_argument('--log_path', type=str, default='./log_path_adrone_foggy2', help="log dir ")
