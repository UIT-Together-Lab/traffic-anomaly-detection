...
train_folder = args.dataset_path+"/"+"/train"
...
for epoch in range(args.epochs):
    ...
    start = time.time()
    with tqdm.tqdm(desc='Epoch %d - train' % (epoch + 1), unit='it', total=len(train_batch)) as pbar:
        for j, (imgs) in enumerate(train_batch):
            imgs = Variable(imgs).cuda()
            
            if args.method == 'pred':
                bs, _, _, h, w = imgs.shape
                outputs, _, _, m_items, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss = model.forward(imgs.view(bs, -1, h, w)[:,0:12].float(), m_items, True)
            ...
            optimizer.zero_grad()
            if args.method == 'pred':
                loss_pixel = torch.mean(loss_func_mse(outputs, imgs[:,4:].squeeze(1).float()))
            else:
            ...
            pbar.set_postfix(loss=loss.item() / (j + 1))
            pbar.update()
