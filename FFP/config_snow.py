...
if not os.path.exists('weights_adrone_snow'):
    os.mkdir('weights_adrone_snow')
if not os.path.exists('results'):
    os.mkdir('results')

share_config = {'mode': 'train',
                'dataset': './UIT-ADrone-Snow/data-eval',
                'img_size': (256, 256),
                'data_root': '/andt/'}  # remember the final '/'
...
def update_config(args=None, mode=None):
    ...
    if mode == 'train':
        ...
        share_config['train_data'] = share_config['data_root'] + args.dataset + '/train/'
        share_config['test_data'] = share_config['data_root'] + args.dataset +  '/test/frames/'
        share_config['g_lr'] = 0.0002
        share_config['d_lr'] = 0.00002
        share_config['resume'] = glob(f'weights_adrone_snow_effect/{args.resume}*')[0] if args.resume else None
