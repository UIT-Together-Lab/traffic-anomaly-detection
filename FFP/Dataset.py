...
class Label_loader:
    def __init__(self, cfg, video_folders):
        # assert cfg.dataset in ('ped2', 'avenue', 'AD_dataset_2022'), f'Did not find the related gt for \'{cfg.dataset}\'.'
        self.cfg = cfg
        self.name = cfg.dataset
        self.frame_path = cfg.test_data
        #self.mat_path = f'../{self.name}/{self.name}.mat'
        self.video_folders = video_folders

    def __call__(self):
        # if self.name == 'AD_dataset_2022':
        #     gt = self.load_AD_dataset_2022()
        # else:
    ...
    def load_ucsd_avenue(self):
        ...
        for i in range(abnormal_events.shape[0]):
            ...
            sub_video_gt = np.zeros((abnormal_events.shape[0],), dtype=np.int8)
    ...
    def load_AD_dataset_2022(self):
        np_list = os.listdir(f'{self.cfg.data_root + self.name}/test/frames_foggy/')
        np_list= sorted(np_list)

        gt = []
        for npy in np_list:
            gt.append(np.load(f'{self.cfg.data_root + self.name}/test/test_frame_mask/'+npy + '.npy'))
