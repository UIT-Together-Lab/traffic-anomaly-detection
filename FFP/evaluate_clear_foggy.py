...
def val(cfg, model=None):
    ...
    if model:  # This is for testing during training.
        ...
        print(f'The pre-trained generator has been loaded from \'{cfg.trained_model}\'.\n')
    ...
    f = open("save_score_final_uit_adrone_clear_foggy_" + cfg.test_data.split('/')[1] + ".txt", "w")
    f.write(save_str)
    f.close()
    save_str=""
    save_str=str(list(np.reshape(np.asarray(labels), (1, np.size(labels)))[0]))[1:-1]
    f = open("save_label_final_uit_adrone_clear_foggy_" + cfg.test_data.split('/')[1] + ".txt", "w")
    f.write(save_str)
    f.close()
