...
parser.add_argument('--dataset', default='UIT-ADrone/images-original', type=str, help='The name of the dataset to train.')
...
save_dir = 'weights_uit_adrone_original'
...
try:
        ...
        for indice, clips, flow_strs in train_dataloader:
            ...
            if step != start_iter:
                ...
                if step % train_cfg.save_interval == 0:
                    ...
                    torch.save(model_dict, f'{save_dir}/model_{step}.pth')
                ...
                if step % train_cfg.val_interval == 0:
                    ...
                    if loss < auc:
                        ...
                        model_dict = {'net_g': generator.state_dict(), 'optimizer_g': optimizer_G.state_dict(),
                                    'net_d': discriminator.state_dict(), 'optimizer_d': optimizer_D.state_dict()}
                        torch.save(model_dict, f'{save_dir}/model_final.pth')
                        loss_step=step
                    generator.train()
            ...
            if step > train_cfg.iters:
                ...
                model_dict = {'net_g': generator.state_dict(), 'optimizer_g': optimizer_G.state_dict(),
                                'net_d': discriminator.state_dict(), 'optimizer_d': optimizer_D.state_dict()}
                torch.save(model_dict, f'{save_dir}/latest_model_{step}.pth')
                break
    ...
    if glob(f'weights/latest*'):
        os.remove(glob(f'{save_dir}/latest*')[0])
