...
if not os.path.exists('weights_uit_drone_original'):
    os.mkdir('weights_uit_drone_original')
if not os.path.exists('results'):
    os.mkdir('results')

share_config = {'mode': 'train',
                'dataset': './UIT-ADrone/images-original',
                'img_size': (256, 256),
                'data_root': '/mnt/UIT-Together/'}  # remember the final '/'
...
def update_config(args=None, mode=None):
    ...
    if mode == 'train':
        ...
        share_config['train_data'] = share_config['data_root'] + args.dataset + '/train/frames'
        share_config['test_data'] = share_config['data_root'] + args.dataset + '/test/frames/'
        share_config['g_lr'] = 0.0002
        share_config['d_lr'] = 0.00002
        share_config['resume'] = glob(f'weights_uit_drone_original/{args.resume}*')[0] if args.resume else None
