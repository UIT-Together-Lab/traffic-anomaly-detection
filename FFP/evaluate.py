...
#np_list = os.listdir("/home/abnormal_detection/VuNgocTu/dataset/AD_dataset_2022tech/testing/test_frame_mask/")
#np_list=sorted(np_list)
...
def val(cfg, model=None):
    ...
    if model:  # This is for testing during training.
        ...
        generator.load_state_dict(torch.load(cfg.trained_model)['net_g'])
        print(f'The pre-trained generator has been loaded from \'weights_drone_anomaly_bike_roundabout_foggy1/{cfg.trained_model}\'.\n')
        ...
        for i, folder in enumerate(video_folders):
            ...
            if not model:
                ...
                if cfg.show_curve:
                    ...
                    predict_writer = cv2.VideoWriter(f'results/{name}_video_predict.avi', fourcc, 30, cfg.img_size)
            ...
            for j, clip in enumerate(dataset):
                ...
                if not model:
                    if cfg.show_curve:
                        ...
                        pred=G_frame.cpu().detach().numpy()
                        #print(pred.shape)
                        predict_frame=((pred[0] + 1) * 127.5).transpose(1, 2, 0).astype('uint8')
                        tar=target_frame.cpu().detach().numpy()
                        tar_frame=((tar[0] + 1) * 127.5).transpose(1, 2, 0).astype('uint8')
                        #cv2.imshow('target frames', cv2_frame)
                        #cv2.waitKey(1)  # show video
                        predict_writer.write(predict_frame)
    ...
    save_str=""
    save_str=str(list(np.reshape(np.asarray(scores), (1, np.size(scores)))[0]))[1:-1]
    f = open("save_score_final_snow_effect_" + cfg.test_data.split('/')[1] + ".txt", "w")
    f.write(save_str)
    f.close()
    save_str=""
    save_str=str(list(np.reshape(np.asarray(labels), (1, np.size(labels)))[0]))[1:-1]
    f = open("save_label_final_snow_effect_" + cfg.test_data.split('/')[1] + ".txt", "w")
    f.write(save_str)
    f.close()
